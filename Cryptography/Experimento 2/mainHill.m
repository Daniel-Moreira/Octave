...
% 'NotesUnderground.txt' and Hill cypher
...
for tamanho=1:Nsize
  ...
  cifra = hillE(plain,reshape(keys(keyRealInd,:), 2, 2));
  

    % encontra a verossimilhanca de toda chave
	probs = mlHillDigrama(freq,keys,cifra);
	%probs = mlHillMonograma(freq,keys,cifra);
...
print -dpng lab2-hill.png
save lab2-hill.dat  probMax probReal entropia acertos